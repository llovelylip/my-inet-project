/*
 CREATE TABLE ICHIP_CATEGORY (
 	ID NUMBER(10),
 	NAME VARCHAR2(30),
 	PARENT_ID NUMBER(10),
 	GROUP_ID NUMBER(5),
 	INDENT_LEVEL NUMBER(5),
 	STATUS NUMBER(1))  
 	CREATE SEQUENCE ICHIP_CATEGORY_SEQ INCREMENT BY 1 START WITH 1 ORDER NOCYCLE;
 */
...
public class CategoryDAO {
    ...
    public boolean insertRow(String name, BigDecimal parentId, int groupId, int indentLevel, int status) {
        ...
        try {
            ...
            strSQL =
                "INSERT INTO ICHIP_CATEGORY (ID, NAME, PARENT_ID, GROUP_ID, INDENT_LEVEL, STATUS) " +
                "VALUES (ICHIP_CATEGORY_SEQ.nextval, ?, ?, ?, ?, ?)";
    ...
    public boolean deleteRow(BigDecimal id) {
        ...
        try {
            conn = poolX.getConnection();
            strSQL =
                "delete from ICHIP_CATEGORY where ID = ?";
            preStmt = conn.prepareStatement(strSQL);
    ...
    public Category getRow(BigDecimal id) {
        Connection conn = null;
        PreparedStatement preStmt = null;
        ResultSet rs = null;
        String strSQL = null;
    ...
    public boolean updateRow(Category cat) {
        if (cat == null) {
            logger.log("ICHIP_Category is null --> could not be updated");
            return false;
        }
        ...
        try {
            conn = poolX.getConnection();
            strSQL =
                "UPDATE ICHIP_ATEGORY SET NAME = ?, PARENT_ID = ?, STATUS = ? " +
                "WHERE ID = ?";
            preStmt = conn.prepareStatement(strSQL);
    ...
    public Collection findAllRoots() {
        ...
        try {
            ...
            strSQL =
                "SELECT ID, NAME, PARENT_ID, GROUP_ID, INDENT_LEVEL, STATUS " +
                "FROM ICHIP_CATEGORY WHERE PARENT_ID = 0";
            preStmt = conn.prepareStatement(strSQL);
            rs = preStmt.executeQuery();
            result = new Vector();
            Category cat = null;
    ...
    public Collection findAllRoots(int status) {
        ...
        try {
            ...
            strSQL =
                "SELECT ID, NAME, PARENT_ID, GROUP_ID, INDENT_LEVEL, STATUS " +
                "FROM ICHIP_CATEGORY WHERE PARENT_ID = 0 AND STATUS = ?";
            preStmt = conn.prepareStatement(strSQL);
    ...
    public Collection findByParentId(BigDecimal parentId) {
        ...
        try {
            ...
            strSQL =
                "SELECT ID, NAME, PARENT_ID, GROUP_ID, INDENT_LEVEL, STATUS " +
                "FROM ICHIP_CATEGORY WHERE PARENT_ID = ?";
            preStmt = conn.prepareStatement(strSQL);
    ...
    public Collection findByParentId(BigDecimal parentId, int status) {
        ...
        try {
            ...
            strSQL =
                "SELECT ID, NAME, PARENT_ID, GROUP_ID, INDENT_LEVEL, STATUS " +
                "FROM ICHIP_CATEGORY WHERE PARENT_ID = ? AND STATUS = ?";
            preStmt = conn.prepareStatement(strSQL);
            preStmt.setBigDecimal(1, parentId);
            preStmt.setInt(1, status);
            rs = preStmt.executeQuery();
    ...
    public int getNewGroupId() {
        ...
        try {
            conn = poolX.getConnection();
            strSQL =
                "SELECT MAX(GROUP_ID) FROM ICHIP_CATEGORY";
            preStmt = conn.prepareStatement(strSQL);
            rs = preStmt.executeQuery();
            if (rs.next()) {
                groupId = rs.getInt(1);
    ...
    public int getCountChidren(BigDecimal id) {
        ...
        try {
            conn = poolX.getConnection();
            strSQL =
                "SELECT COUNT(*) FROM ICHIP_CATEGORY WHERE PARENT_ID = ?";
            preStmt = conn.prepareStatement(strSQL);
            preStmt.setBigDecimal(1, id);
            rs = preStmt.executeQuery();
            if (rs.next()) {

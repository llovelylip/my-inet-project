...
public class MultipartRequest {
    ...
    public static class File {
         ...
         * in lowercase.
         *
         * @return  the MIME type of the file or an empty string if the type is not known
         */
    ...
    private Map parseHeader(String header) {
        String token; // a token is a name|key - value pair
        String key;
        String value;
        int delimiter;
        ...
        if (header == null)
            header = "";
        while (!header.equals("")) {
            //Each Header:   <name|key>:<value> {;attributeName=attributeValue}
    ...
    private void parseRequest(HttpServletRequest req)
        ...
        while(c != -2) {
            ...
            out.close();
            if ( file == null )
                putParameter( name, ((ByteArrayOutputStream)out).toString("utf-8"));//("iso-8859-1") );
        }
    }
     ...
     * @throws  IOException   if an error occurs while reading the request
     */
    private String readLine( PushbackInputStream input )
        throws IOException {
     ...
     */
    private void unread( int b, PushbackInputStream input )
        throws IOException {
        unreadCount++;
        input.unread( b );
    }
